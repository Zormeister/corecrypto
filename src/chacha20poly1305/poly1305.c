...
int ccpoly1305_final(ccpoly1305_ctx *ctx, void *tag)
{
	...
	/* mac = (h + pad) % (2^128) */
	f = (uint64_t)h0 + ctx->key[0]            ; h0 = (uint32_t)f;
	f = (uint64_t)h1 + ctx->key[1] + (f >> 32); h1 = (uint32_t)f;
	f = (uint64_t)h2 + ctx->key[2] + (f >> 32); h2 = (uint32_t)f;
	f = (uint64_t)h3 + ctx->key[3] + (f >> 32); h3 = (uint32_t)f;
