...
extern void AESDecryptCBC(void *out, const void *in, void *iv, const uint32_t *expanded_key, long nblocks, int nrounds);
...
static int cbc_opt_wrapper_dec(const cccbc_ctx *ctx, cccbc_iv *iv, size_t nblocks, const void *in, void *out) {
    struct ccaes_intel_opt_key *k = (struct ccaes_intel_opt_key *)ctx;
    AESDecryptCBC(out, in, iv->b, k->key, nblocks, k->rounds);
    return 0;
}
