//
//  ccaes_intel_ecb_encrypt_opt_mode.c
//  corecrypto
//
...
extern void AESEncryptWithExpandedKey(void *out, const void *in, const uint32_t *expanded_key, int nrounds);
...
static int opt_wrapper_enc(const ccecb_ctx *ctx, size_t nblocks, const void *in, void *out)
{
    struct ccaes_intel_opt_key *k = (struct ccaes_intel_opt_key *)ctx;
    while (nblocks--) {
        AESEncryptWithExpandedKey(out, in, k->key, k->rounds);
